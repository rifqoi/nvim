...
function M.get()
	...
	-- global components
	local invi_sep = {str = " ", hl = {fg = sett.bkg, bg = sett.bkg}}

	-- helpers
	local function any_git_changes()
		local gst = b.gitsigns_status_dict -- git stats
		if gst then
			if gst["added"] and gst["added"] > 0 or gst["removed"] and gst["removed"] > 0 or
							gst["changed"] and gst["changed"] > 0 then return true end
		end
		return false
	end
	...
	local vi_mode_hl = function()
		return {fg = sett.text, bg = mode_colors[vim.fn.mode()][2], style = "bold"}
	end

	components.active[1][1] = {
		provider = assets.bar,
		hl = function() return {fg = mode_colors[vim.fn.mode()][2], bg = sett.bkg} end,
	}

	components.active[1][2] = {
		provider = assets.mode_icon,
		hl = function() return {fg = sett.text, bg = mode_colors[vim.fn.mode()][2]} end,
	}
	...
	components.active[1][5] = {
		provider = assets.right_separator,
		hl = function() return {fg = mode_colors[vim.fn.mode()][2], bg = sett.diffs} end,
	...
	components.active[2][1] = {
		...
		hl = {fg = clrs.rosewater, bg = sett.bkg},
	}
	...
	components.active[3][3] = {
		provider = function()
			...
			return
							(sett.show_modified and "%m" or "") .. " " .. icon .. " " .. filename ..
											" "
		end,
		enabled = is_enabled(70),
		hl = {fg = sett.text, bg = sett.curr_file},
		left_sep = {
			str = assets.left_separator,
			hl = {fg = sett.curr_file, bg = sett.bkg},
		},
	}
	...
	components.active[3][4] = {
		...
		enabled = is_enabled(80),
		hl = {fg = sett.text, bg = sett.curr_dir},
		left_sep = {
			str = assets.left_separator,
			hl = {fg = sett.curr_dir, bg = sett.curr_file},
		},
	}
	-- ######## Right
	...
	components.inactive[1][1] = {
		provider = function() return " " .. string.upper(vim.fn.expand('%:t')) .. " " end,
		hl = {fg = clrs.overlay2, bg = clrs.mantle},
	}
