...
local ViMode = {
	...
	update = {"ModeChanged", callback = function() vim.cmd("redrawstatus") end},
}
...
local Gps2 = {
	...
	init = function(self)
		...
		for i, d in ipairs(data) do
			local child = {
				{provider = d.icon, hl = self.type_map[d.type]},
				{provider = d.text},
			}
...
local Git = {
	...
	init = function(self)
		...
		self.has_changes = self.status_dict.added ~= 0 or self.status_dict.removed ~=
...
local CloseButton = {
	...
	{provider = " "},
	{
		...
		hl = {fg = "gray"},
		on_click = {
			callback = function(_, winid) vim.api.nvim_win_close(winid, true) end,
			name = function(self) return "heirline_close_button_" .. self.winnr end,
			update = true,
		},
	},
}
...
local WinBar = {
	...
	},
	{
		...
		utils.surround({"", ""}, "dark_red",
		               {FileType, Space, TerminalName, CloseButton}),
	},
	utils.surround({"", ""}, "bright_bg", {
		hl = function()
			if not conditions.is_active() then return {fg = "gray", force = true} end
		end,
...
vim.api.nvim_create_autocmd("User", {
	...
	callback = function(args)
		local buf = args.buf
		local buftype = vim.tbl_contains({"prompt", "nofile", "help", "quickfix"},
		                                 vim.bo[buf].buftype)
		local filetype = vim.tbl_contains({"gitcommit", "fugitive"},
		                                  vim.bo[buf].filetype)
...
vim.api.nvim_create_autocmd("ColorScheme", {
	callback = function()
		...
		require("heirline").statusline:broadcast(
						function(self) self._win_stl = nil end)
	end,
	group = "Heirline",
})
